...
export default function UserList() {
  ...
  const handleUpdateBalance = async (uid, newBalance) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_CLOUD_FUNCTIONS_URL}/adminUpdateBalance`,
        {
          method: 'POST',
  ...
  const handleToggleBan = async (uid, currentStatus) => {
    try {
      ...
      showToast(
        !currentStatus ? 'User banned' : 'User unbanned',
          ...
          <div key={uid} className="glass-card rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2 ml-2">
                ...
                {data.isAdmin && (
                  <span className="px-2 py-0.5 rounded-full bg-gold-500/10 text-gold-400 text-[10px] font-medium">
                    Admin
                  </span>
                )}
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs mb-3">
              <div>
                <p className="text-gold-400 font-semibold">{data.balance?.toFixed(2) || '0.00'}</p>
                <p className="text-gray-500">Balance</p>
              </div>
              <div>
                <p className="text-blue-400 font-semibold">{data.adsWatched || 0}</p>
                <p className="text-gray-500">Ads</p>
              </div>
              <div>
                <p className="text-emerald-400 font-semibold">{data.referrals || 0}</p>
                <p className="text-gray-500">Refs</p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-1.5 rounded-lg bg-gold-500/10 text-gold-400 text-xs font-medium hover:bg-gold-500/20 transition-colors"
              >
                Edit Balance
              </button>
              <button
                onClick={() => handleToggleBan(uid, data.isBanned)}
              ...
              <div className="mt-3 pt-3 border-t border-white/10">
                <div className="flex gap-2">
                  <input
                    ...
                    step="0.01"
                    min="0"
                    className="flex-1 bg-dark-700 border border-white/10 rounded-lg px-3 py-1.5 text-white text-sm outline-none focus:border-gold-500"
                    autoFocus
                  />
                  <button
                    onClick={() => handleUpdateBalance(uid, editBalance)}
                    className="px-3 py-1.5 rounded-lg gold-gradient text-dark-950 text-xs font-bold"
                  >
                    Save
                  </button>
                  <button
